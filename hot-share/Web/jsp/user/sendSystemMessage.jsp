...
.rt2 {
	...
	$(function() {
		...
		$(":submit").click(function() {
			...
			$.post("${basePath}/admin/systemMessage/sendSystemMessage", {
				ids : '${ids}',
				messageId : $("#systemMessageList").val()
			}, function(data, textStatus) {
				if (data.success) {
					$.messager.alert('操作提示', "发送系统消息成功", 'info');
					$("#returnSystemMessage").trigger("click");
				} else if (data.message) {
					$.messager.alert('操作提示', data.message, 'info');
				} else {
					$.messager.alert('操作提示',"发送系统消息异常，请联系相关技术人员", 'error');
				}
			}, "json");
			return false;
		});
			...
			<form id="ff"
				...
				<table
					...
					<tr>
						...
						<td class="rt2">
							<select class="easyui-validatebox formtext" name="systemMessageList"
								id="systemMessageList">
								<c:forEach var="systemMessage" items="${systemMessageList}">
									<option value="${systemMessage.id}">
										${systemMessage.title}
									</option>
								</c:forEach>
							</select>
