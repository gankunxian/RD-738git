...
.rt2 {
	...
	width: auto;
		...
		<script type="text/javascript">
	$(function() {
		$("#title").val('${systemMessage.title}');
		$("#content").val('${systemMessage.content}');
		$("#remark").val('${systemMessage.remark}');
		...
		$(":submit").click(function() {
			...
			$.post("${basePath}/admin/systemMessage/editSystemMessage", {
			    id: '${systemMessage.id}',
				title : $("#title").val(),
				content : $("#content").val(),
				remark : $("#remark").val(),
				...
				if (data.success) {
					$.messager.alert('操作提示', "编辑系统消息成功", 'info');
					$("#returnSystemMessage").trigger("click");
				} else if (data.message) {
					$.messager.alert('操作提示', data.message, 'info');
				} else {
					$.messager.alert('操作提示', "编辑系统消息异常，请联系相关技术人员", 'error');
				}
			}, "json");
			return false;
		});
...
.combo input[type="text"] {
	...
	<body class="easyui-layout">
		<div region="center" title="编辑系统消息" iconCls="icon-add">
			<form id="ff"
