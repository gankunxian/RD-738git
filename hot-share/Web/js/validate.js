$.extend($.fn.validatebox.defaults.rules, {
	...
	mobile : {// 验证手机号码
		validator : function(value) {
			return /^1[3|4|5|8][0-9]\d{8}$/i.test(value);
		},
		message : '手机号码格式不正确'
	},
	idcard : {// 验证身份证
	...
	ip : {// 验证IP地址
		...
		message : 'ipScope输入格式不正确'
	},
		caIdScope : {// 验证caIdScope格式
		validator : function(value) {
			return /^\d+-\d+(;\d+-\d+)*$/i.test(value);
		},
		message : 'caIdScope输入格式不正确'
	},
	name : {// 验证姓名，可以是中文或英文
		validator : function(value) {
			return /^[\u0391-\uFFE5]+$/i.test(value)
	...
	notzero : {
		validator : function(value, param) {
			var selected = $("input[name='" + param + "']").val();
