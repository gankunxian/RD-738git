...
public class VerifyCodeImageServlet extends HttpServlet {
	...
	// 定义图形验证码中绘制字符的字体
	private final Font mFont = new Font("Arial Black", Font.PLAIN, 30);

	// 定义图形验证码的大小
	private final int IMG_WIDTH = 130;

	private final int IMG_HEIGTH = 38;
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < 4; i++) {
			...
			// 在图片上绘制系统生成的随机字符
			g.drawString(tmp, 30 * i + 10, 30);
		}
		// 获取HttpSesssion对象
		HttpSession session = request.getSession(true);
		// 将随机字符串放入HttpSesssion对象中
		session.setAttribute(Constant.HOT_SHARE_MANAGE_VERIFY_CODE, sRand);
		g.dispose();
		// 向输出流中输出图片
